...
def send_mail_tasks(username, email, token):
    ...
    html_message = f'<h3>您好,{username}:</h3><br><h5>请点击链接完成注册激活:<br><a href="http://127.0.0.1:8000/api_user/active_user/{token}">http://127.0.0.1:8000/user/active/{token}</a><br>激活后才可以登录账号，请在5分钟之内激活否则链接失效</h5>'  # 写html代码， 可以转意
