...
class BaseModel(object):
    ...
    update_time = db.Column(db.DateTime, default=datetime.now, onupdate=datetime.now)  # 记录的更新时间对密码加密
    delete_mark = db.Column(db.String(10), default="0")  # 删除标记
...
class User(BaseModel, db.Model):
    ...
    id_card = db.Column(db.String(80))  # 身份证号
    ...
    def to_dict(self):
        ...
        user_dict = {
            ...
            "name": self.name,
            "mobile": self.mobile,
            "avatar": controllerCode.FDFS_BASE_URL + self.avatar_url if self.avatar_url else "",
...
# 房屋设施表，建立房屋与设施的多对多关系
'''house_id和facility_id 组成主键就不用id了'''
...
class House(BaseModel, db.Model):
    ...
    facilities = db.relationship("Facility", secondary=house_facility)  # 房屋的设施,关系
    ...
    def to_basic_dict(self):
        ...
        house_dict = {
            ...
            "img_url": controllerCode.FDFS_BASE_URL + self.index_image_url if self.index_image_url else "",
            "room_count": self.room_count,
            "order_count": self.order_count,
            "address": self.address,
            "user_avatar": controllerCode.FDFS_BASE_URL + self.user.avatar_url if self.user.avatar_url else "",
    ...
    def to_full_dict(self):
        ...
        for image in self.images:
            img_urls.append(controllerCode.FDFS_BASE_URL + image.url)
        ...
        orders = Order.query.filter(Order.house_id == self.id, Order.status == "COMPLETE", Order.comment != None)\
            .order_by(Order.update_time.desc()).limit(controllerCode.HOUSE_DETAIL_COMMENT_DISPLAY_COUNTS)
        for order in orders:
            comment = {
...
class Order(BaseModel, db.Model):
    ...
    def to_dict(self):
        ...
        order_dict = {
            ...
            "img_url": controllerCode.FDFS_BASE_URL + self.house.index_image_url if self.house.index_image_url else "",
